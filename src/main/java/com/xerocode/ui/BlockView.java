...
public final class BlockView {
    ...
    public static void ghost(GuiGraphicsExtractor ctx, Layout.Box box, int argb) {
    ...
    public static void paint(GuiGraphicsExtractor ctx, Font tr, Layout layout,
    ...
    private static void card(GuiGraphicsExtractor ctx, Font tr, Layout.Box box, Look look,
    ...
    private static void plusChip(GuiGraphicsExtractor ctx, Font tr, Layout.Chip chip) {
    ...
    private static void argChip(GuiGraphicsExtractor ctx, Font tr, Layout.Chip chip) {
        Draw.pill(ctx, chip.x, chip.y, chip.w, Layout.CHIP_H, chip.border);
        Draw.pillGrad(ctx, chip.x + 1, chip.y + 1, chip.w - 2, Layout.CHIP_H - 2,
                chip.top, chip.bottom);
    ...
    public static void badge(GuiGraphicsExtractor ctx, int x, int y, ItemStack icon, int dot) {
        if (icon == null || icon.isEmpty()) Draw.dot(ctx, x + 5, y + 5, dot);
        else Draw.item(ctx, icon, x + 2, y + 2, 11);
    }

    private static void chipBadge(GuiGraphicsExtractor ctx, Layout.Chip chip, ItemStack icon, int dot) {
    ...
    public static int pill(GuiGraphicsExtractor ctx, int x, int y, int w, int face,
    ...
    private static int chipPill(GuiGraphicsExtractor ctx, Layout.Chip chip, int face, boolean tinted) {
        return pill(ctx, chip.x, chip.y, chip.w, face, tinted, 0xFF);
    }

    private static void conditionChip(GuiGraphicsExtractor ctx, Font tr, Layout.Box box,
                                      Layout.Chip chip) {
    ...
    private static void markerChip(GuiGraphicsExtractor ctx, Font tr, Layout.Box box,
                                   Layout.Chip chip) {
